...
/**
 * NVIDIA NIM adapter — OpenAI-compatible API at integrate.api.nvidia.com
 * Supports all NIM-hosted models (LLMs, vision, etc.)
 * For Cosmos video generation, use the cosmos-video edge function instead.
 */
export const nvidiaNimAdapter: ProviderAdapter = {
  ...
  async parseResponse(resp) {
    ...
    if (data.error) throw new Error(data.error.message || data.error || "NIM API error");
    return {
  ...
  parseStreamLine(line) {
    ...
    try {
      ...
      if (data.error) return { content: `[Error: ${data.error.message}]`, done: true };
